...
class ConvImageEmbedding(nn.Module):
    """
    Convolutional image embedding for kxk views
    image_size: k
    """

    def __init__(self, input_channels, hidden_size, embedding_size, image_size=3):
        super(ConvImageEmbedding, self).__init__()
        self.image_size = image_size
        self.conv1 = nn.Conv2d(input_channels, hidden_size, kernel_size=2, stride=1, padding=0)
        self.bn1 = nn.BatchNorm2d(hidden_size)
        self.conv2 = nn.Conv2d(hidden_size, hidden_size * 2, kernel_size=2, stride=1, padding=0)
        self.bn2 = nn.BatchNorm2d(hidden_size * 2)
        if self.image_size > 3:
            self.conv3 = nn.Conv2d(hidden_size * 2, hidden_size * 4, kernel_size=2, stride=1, padding=0)
            self.bn3 = nn.BatchNorm2d(hidden_size * 4)
        # self.pool = nn.AdaptiveMaxPool2d((1, 1))  # pooling destroys spatial information - not good for our case
        self.flatten = nn.Flatten()
        if self.image_size > 3:
            remaining_size = image_size - 3
            self.linear = nn.Linear(hidden_size * 4 * remaining_size * remaining_size, embedding_size)
        else:
...
class MemoryGridNet(nn.Module):
    def __init__(self, observation_shape, num_actions, use_lstm=False):
        super(MemoryGridNet, self).__init__()
        # self.observation_shape = observation_shape
        self.observation_shape = observation_shape
        self.num_actions = num_actions
        hidden_size = 64
        ...
        # self.conv2 = nn.Conv2d(hidden_size, hidden_size * 2, stride=1, kernel_size=2, padding=0),
        # self.fc = nn.Linear(hidden_size * 2, embedding_size)
        # Feature extraction.
        # self.conv1 = nn.Conv2d(
        #     in_channels=self.observation_shape[0],
        #     out_channels=32,
        ...
        # self.conv2 = nn.Conv2d(32, 64, kernel_size=4, stride=2)
        # self.conv3 = nn.Conv2d(64, 64, kernel_size=3, stride=1)
        self.conv = ConvImageEmbedding(self.observation_shape[0], hidden_size, fc_out_size, image_size=self.observation_shape[1])

        # self.fc = nn.Linear(3136, 512)

        # FC output size + one-hot of last action + last reward.
        core_output_size = fc_out_size + num_actions + 1
